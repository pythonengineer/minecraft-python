...
class RenderHelper:
    ...
    def enableStandardItemLighting():
        ...
        vec = Vec3D(0.0, 1.0, -1.0)
        RenderHelper.__setColorBuffer(vec.xCoord, vec.yCoord,
                                      vec.zCoord, 0.0).glLightfv(gl.GL_LIGHT1, gl.GL_POSITION)
        RenderHelper.__setColorBuffer(y, y, y, 1.0).glLightfv(gl.GL_LIGHT1, gl.GL_DIFFUSE)
        RenderHelper.__setColorBuffer(0.0, 0.0, 0.0, 1.0).glLightfv(gl.GL_LIGHT1, gl.GL_AMBIENT)
        RenderHelper.__setColorBuffer(z, z, z, 1.0).glLightfv(gl.GL_LIGHT1, gl.GL_SPECULAR)
